...
const testEmployee = new Manager("Warfa", "2932374" , "warfa60@gmail.com", "614");

test("testing if new manager object will work", () => {
    expect(testEmployee.name).toBe("Warfa");
    expect(testEmployee.id).toBe("2932374");
    expect(testEmployee.email).toBe("warfa60@gmail.com");
    expect(testEmployee.officeNumber).toBe("614");
});
...
test("This test gets the office number from getOfficeNumber", () => {
    expect(testEmployee.getOfficeNumber()).toBe("614");
});
